...
const detectModuleType = require('../utilities/detect-module-type');
const detectModuleStructure = require('../utilities/detect-module-structure');
const detectModuleName = require('../utilities/detect-module-name');
const getTypeName = require('../utilities/get-type-name');
...
module.exports = Task.extend({
  run: function (options) {
    ...
    this.visitTemplates(templates, visitOptions).forEach(function (tree) {
      if (tree) {
        const printed = templateRecast.print(tree.ast).code;
        if (!options.dryRun) {
          ...
          if (typeof node.value === 'string') {
            options.sourceNameVariations.forEach(function (variation, index) {
              ...
              if (variationIndex !== -1) {
                ...
                if (
                  variationIndex === 0 ||
                  node.value.charAt(variationIndex - 1) === ' '
                ) {
                  hasSourceRef = true;
                  ...
                  */
                  ...
                  segment = node.value.substr(
                    typeIndex + options.sourceTypeName.length + 1,
                    node.value.length
                  );
  ...
  transformComponentName: function(fileInfo, config, invokableData) {
    ...
    const srcComponentPath = componentNameUtils.componentPathFromProjectPath(
      config.sourcePath
    );
    const sourceTag = componentNameUtils.componentPathToTag(srcComponentPath);
    const destComponentPath = componentNameUtils.componentPathFromProjectPath(
      config.destPath
    );
    const destTag = componentNameUtils.componentPathToTag(destComponentPath);
    return {
      ElementNode(node) {
        if (node.tag === sourceTag) {
          debug('Found tag to replace!', sourceTag);
          debug('sourceTag', sourceTag);
