    ...
    return {
      ElementNode(node) {
        if (node.tag === sourceTag) {
          debug('Found angle-brackets component to replace', sourceTag);
      ...
      MustacheStatement(ms) {
        if (ms.path.original === config.sourcePath) {
          debug('Found mustache component to replace', ms.path.original);
          ms.path.original = config.destPath;
          ms.path.parts = [config.destPath];
          return b.mustache(
            ms.path,
            ms.params,
            ms.hash,
